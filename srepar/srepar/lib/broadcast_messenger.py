"""
Source: pyro/poutine/broadcast_messenger.py (Pyro 1.3.0)
Edits done:
- In the original `_pyro_sample`, the line `msg["fn"].has_rsample = dist.has_rsample`
  raises the error `AttributeError: can't set attribute` when `msg["fn"]` is `Independent`
  distribution, since `has_rsample` of `Independent` class cannot be set directly.
- In the below `_my_pyro_sample`, this issue is fixed: instead of directly setting
  `msg["fn"].has_rsample`, `msg["fn"].base_dist.has_rsample` is set.
"""
# Copyright (c) 2017-2019 Uber Technologies, Inc.
# SPDX-License-Identifier: Apache-2.0

from pyro.util import ignore_jit_warnings
from pyro.poutine.messenger import Messenger # WL: edited path.
from . import utils as my_utils # WL: added.
...
class BroadcastMessenger(Messenger):
    ...
    def _my_pyro_sample(msg): # WL: edited func name.
        """
        ...
        if actual_batch_shape is not None:
            ...
            if msg["fn"].has_rsample != dist.has_rsample:
                ...
                my_utils.set_has_rsample(msg["fn"], dist.has_rsample)
                # assert(msg["fn"].has_rsample == dist.has_rsample)
                # ====================
